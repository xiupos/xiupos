...
Dirac のデルタ関数の概形はよく「分散 0 の正規分布」と言い表される. 関数形を比較すると直感的には正しそうに思えるが, ここでは実際に計算によって確かめてみよう. 数学的な厳密さは気にしない.
...
$$
となる. ここで, 関数 $f$ は任意だから $\mathcal{F}[δ] = 1$, つまりデルタ関数は恒等的に1を返す関数の Fourier 逆変換 $δ = \mathcal{F}^{-1}[1]$ であることがわかる[^delta_fourier]. 以上より, 有名な積分による表式
$$
...
$$
f_{m,s^2}(x) ≡ \frac{e^{-(x-m)^2/2s^2}}{\sqrt{2πs^2}}
$$
で与えられる. 天下り的であるが, 平均 $0$ の正規分布に対して Fourier 変換を実行すると
$$
\mathcal{F}[f_{m=0,s^2}](k) = \frac1{\sqrt{2πs^2}} ∫ \dd{x}\ e^{- x^2/2s^2 - ikx} = e^{-s^2k^2/2}
$$
であるから, 元の確率密度関数は逆変換より
$$
f_{0,s^2}(x) = ∫ \frac{\dd{k}}{2π} e^{-s^2k^2/2} e^{ikx}
$$
と書ける. この表式の下, 分散を形式的に $s^2 → +0$ とすれば
$$
f_{0,s^2=+0}(x) = ∫ \frac{\dd{k}}{2π} e^{ikx}
$$
となって, 前述のデルタ関数の表式と等しくなる. 以上より, デルタ関数は分散 $0$ の正規分布の確率密度関数と等しいこと
$$
δ(x) = \lim_{s^2→+0} \frac{e^{-x^2/2s^2}}{\sqrt{2πs^2}}
$$
が確かめられた.
...
[^delta_fourier]: 遠回しな導出をしたが, 単純にデルタ関数の Fourier 変換を考えれば, 定義から直ちにこの結果が得られる.

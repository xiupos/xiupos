...
4元座標 $x = (x^μ)$ に依存する場 $ϕ^α(x)$ に対して, **作用** action と呼ばれる汎関数 $S[ϕ^α]$ が存在し, 物理現象において作用 $S[ϕ^α]$ が最小となるような場が選ばれる. つまり, $ϕ^α(x) ↦ ϕ^α(x) + δϕ^α(x)$ (ただし境界固定 $x∈∂M ⇒ δϕ^α(x)=0$, $M$ は考えている空間) となる変換に対し, 作用が停留値を取る:
$$

...
この記事は, [解析力学](https://xiupos.net/blog/note/particle-c/)において「変分」を用いて導出される諸々を, [汎関数計算](https://xiupos.net/blog/note/functional/)を用いて書き直す試みである. 正確性は保証しない.
...
\end{aligned}
$$
ここで, 最後の式の第一項は Euler–Lagrange の運動方程式より消え, 第二項の $t_1$, $t_2$ は任意である[^noether]. したがって, $δS=0$ であるとすると, 対応する保存量が得られる:
...
$$
また, $δq^j(t)$, $δ\.q^j(t)$ は任意であるから, $δq^j(t)$ と $δp_j(t)$ を独立に取ることができる. したがって, 条件 $δS[q^i] = 0$ より, $t_1<t<t_2$ で次の運動方程式が得られる:

...
\end{aligned}
$$
これは, 端点を固定した経路の経路上各点について積分した積になっていることから, **経路積分**とも呼ばれる. 経路積分の表記法については[別記事](./path-integral-notation)を参照.
...
1. 汎函数部分積分:
    $$
    ∫_{φ_0}^φ \mathcal{D}φ(x) \fdv{F[φ(x’)]}{φ(x)} G[φ(x)]
    = - ∫_{φ_0}^φ \mathcal{D}φ(x) F[φ(x)] \fdv{G[φ(x’)]}{φ(x)}
    $$
    ただし, 

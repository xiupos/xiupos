...
物理では定義域が関数族であるような関数を**汎関数** *functional* と呼ぶ. 例えば, $F:\{A→B\}→C$ など. このとき, $φ:A→B$ に対して $F[φ(x)]∈C$ と書くことが多い. ただし表記中 $x∈A$ は「ダミー」であって, 汎関数の定義中で用いられる変数を明示しているだけに過ぎない. 単に $F[φ]$ と書かれることもある. この文章中では, ダミー変数を添字にした $F_x[φ]$, $F_{x∈A}[φ]$ も用いる[^suffix]. $F[φ(x)]$ が汎関数であるとき, 通常の関数 $g:C→D$ を合成した $g(F[φ(x)])$ もまた汎関数である.

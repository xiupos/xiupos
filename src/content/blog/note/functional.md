...
ここでは定義域が関数であるような関数を汎関数とする. 例えば, $F:(A→B)→C$ など. このとき, $φ:A→B$ を用いて $F[φ(x)]∈C$ と書く. ただし表記中 $x∈A$ は「ダミー」であって, 汎関数の定義中で用いられる文字である. 誤解が無いとき $F[φ]$ と略記される. $F[φ(x)]$ が汎関数であるとき, 通常の関数 $g:C→D$ を用いた $g(F[φ(x)])$ もまた汎関数である.

以下では物理において頻出する汎関数の基本的な計算方法についてまとめる. 数学的な厳密性は一切考慮しない.

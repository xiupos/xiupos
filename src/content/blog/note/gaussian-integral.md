...
最も簡単な形の Gauss 積分は
$$
∫ \dd{x}\ e^{- \frac12 x^2} = \sqrt{2π}
$$
である[^interval]. 実際, $I = ∫ \dd{x}\ e^{-\frac12x^2}$ とすれば,
$$
I^2 = ∫ \dd{x} ∫ \dd{y}\ e^{- \frac12 (x^2 + y^2)} = ∫_0^{2π} \dd{θ} ∫_0^∞\dd{r}\ re^{- \frac12 r^2} = 2π \bqty{-e^{- \frac12 r^2}}_0^∞ = 2π
$$
となって, $I = \sqrt{2π}$ がわかる.
...
- $\displaystyle ∫ \dd{x}\ e^{-\frac12ax^2} = \sqrt{\frac{2π}{a}}$. $\quad (a>0)$  
    ∵ 変数変換 $y = \sqrt{a}x$, $\dd{y} = \sqrt{a} \dd{x}$ して $y$ で積分.

- $\displaystyle ∫ \dd{x}\ e^{-\frac12a(x-x_0)^2} = \sqrt{\frac{2π}{a}}$. $\quad (a>0)$  
    ∵ 変数変換 $y=x-x_0$, $\dd{y} = \dd{x}$ して $y$ で積分.

- $\displaystyle ∫ \dd{x}\ e^{-\frac12ax^2 + bx + c} = \sqrt{\frac{2π}{a}} e^{\frac{b^2}{2a} + c}$. $\quad (a>0)$  
    ∵ 平方完成 $-\frac12ax^2 + bx + c = -\frac12 a\pqty{x - \frac{b}{a}}^2 + \frac{b^2}{2a} + c$ して積分.

また, 係数が複素数であるときにも, いくつかの場合で同様の公式が成立する.

- $\displaystyle ∫\dd{x}\ e^{-\frac12a(x-χ_0)^2} = \sqrt{\frac{2π}{a}}$. $\quad (a>0,\ χ_0∈\mathbb{C})$  
    ∵ $\mathbb{C}$ 平面上 $-R→R→R-β→-R-β→-R$ なる平行四辺形の経路で $e^{-\frac12 az^2}$ を複素積分し, 極限 $R→0$.

- $\displaystyle ∫\dd{x}\ e^{-\frac12α(x-χ_0)^2} = \sqrt{\frac{2π}{α}}$. $\quad (α,χ_0∈\mathbb{C},\ \operatorname{Re} α > 0)$  
    ∵ $e^{-\frac12a(x-β)^2}$ に対する Gauss 積分で $a$ を $\operatorname{Re} α > 0$ に解析接続.

- $\displaystyle ∫\dd{x}\ e^{-\frac12αx^2 + βx + γ} = \sqrt{\frac{2π}{α}} e^{\frac{β^2}{2α} + γ}$. $\quad (α,β,γ∈\mathbb{C},\ \operatorname{Re} α > 0)$  
    ∵ 平方完成 $-\frac12αx^2 + βx + γ = -\frac12 α\pqty{x - \frac{β}{α}}^2 + \frac{β^2}{2α} + γ$ して積分.

- $\displaystyle ∫\dd{x}\ e^{-\frac12ax^2+ikx} = \sqrt{\frac{2π}{a}} e^{-\frac12 \frac{k^2}{a}}$. $\quad (a>0)$  
    ∵ 平方完成 $-\frac12ax^2 + ikx = -\frac12 a\pqty{x - \frac{ik}{a}}^2 - \frac12 \frac{k^2}{a}$ して積分.

- $\displaystyle ∫\dd{x}\ e^{\frac{i}2 a(x-x_0)^2} = \sqrt{\frac{2πi}{a}}. \quad (a≠0)$[^fresnel]  
...
$$
∫\dd{x}\ e^{-\frac12α(x-χ_0)^2} = \sqrt{\frac{2π}{α}}, \quad (α≠0)
$$
である.
...
$$
∫\dd{^N x}\ e^{-\frac12 |\bm{x}|^2} = \sqrt{(2π)^N}
$$
...
$$
∫\dd{^N x}\ e^{-\frac12 |\bm{x}|^2} = \prod_{j=1}^N ∫\dd{x_j}\ e^{-\frac12 x_j^2} = \prod_{j=1}^N \sqrt{2π} = \sqrt{(2π)^N}
$$
となる.
...
- $\displaystyle ∫\dd{^N x}\ e^{-\frac12  \sum_{j=1}^N α_j (x_j-χ_{0j})^2} = \sqrt{\frac{(2π)^N}{\prod_{j=1}^N α_j}}. \quad$ ($α_j≠0$)  
    ∵ 各 $x_j$ について1変数 Gauss 積分. 積分の収束性について気にする場合は, 指数の肩の $α_j$, $χ_{0j}$ についてそれぞれ1変数 Gauss 積分と同様の条件を課せば良い.

- $\displaystyle ∫\dd{^N x}\ e^{-\frac12 (\bm{x}-\bm{x}_0)^⊤ A (\bm{x}-\bm{x}_0)} = \sqrt{\frac{(2π)^N}{\det A}}. \quad$ ($A$ : $N×N$ 実対称行列)  
    ∵ 行列式が正の直交行列 $O$ で $A$ を対角化 $O^{-1}AO = \operatorname{diag}(λ_1,\cdots,λ_N)$ して変数変換 $(\bm{y}-\bm{y}_0)=O^⊤ (\bm{x}-\bm{x}_0)$, $\dd{{}^N y}=\dd{{}^N x}$ すると, 指数の肩が $(\bm{x}-\bm{x}_0)^⊤ A (\bm{x}-\bm{x}_0)$ $= (\bm{y}-\bm{y}_0)^⊤ O^⊤AO (\bm{y}—\bm{y}_0)$ $= \sum_{j=1}^N λ_j (y_j-y_{0j})^2$ と簡単になるから, $\prod_{j=1}^N λ_j = \det A$ に注意して積分. 積分が収束するのは $A$ が正定値であるとき.

- $\displaystyle ∫\dd{^N x}\ e^{-\frac12 (\bm{x}-\bm{x}_0)^⊤ A (\bm{x}-\bm{x}_0)} = \sqrt{\frac{(2π)^N}{\det A}}. \quad$ ($A$ : $N×N$ 実正則行列で対角化可能)  
...
- $\displaystyle ∫\dd{^N x}\ e^{\frac{i}2 (\bm{x}-\bm{x}_0)^⊤ A (\bm{x}-\bm{x}_0)} = \sqrt{\frac{(2πi)^N}{\det A}}. \quad$ ($A$ : $N×N$ 実正則行列で対角化可能)  
    ∵ 上と同様に $A$ を対角化して積分.

- $\displaystyle ∫\dd{^N x}\ e^{-\frac12 (\bm{x}-\bm{x}_0)^⊤ A (\bm{x}-\bm{x}_0)} = \sqrt{\frac{(2π)^N}{\det A}}. \quad$ ($A$ : $N×N$ 複素正則行列で対角化可能)  
    ∵ 同様に $A$ を対称行列と思ってよい. 行列を実部と虚部に分解 $A = A_{\mathrm{R}} + iA_{\mathrm{I}}$ し, $A_{\mathrm{R}}$ を実下三角行列 $C$ で Cholesky 分解 $A_{\mathrm{R}} = CC^⊤$ して変数変換 $(\bm{y}-\bm{y}_0) = C^⊤ (\bm{x}-\bm{x}_0)$, $\dd{{}^N y} = \dd{{}^N x}\sqrt{\det{A_{\mathrm{R}}}}$ すれば $(\bm{x}-\bm{x}_0)^⊤A(\bm{x}-\bm{x}_0)$ $= (\bm{y}-\bm{y}_0)^⊤(\bm{y}-\bm{y}_0)$ $+ i(\bm{y}-\bm{y}_0)^⊤\~A_{\mathrm{I}}(\bm{y}-\bm{y}_0)$. ここで $\~A_{\mathrm{I}}≡CA_{\mathrm{I}}C^⊤$ は実対称行列であるから, 対角化 $O^{-1}\~A_{\mathrm{I}}O = \operatorname{diag}(λ_1,\cdots,λ_N)$ して変数変換 $(\bm{u}-\bm{u}_0) = O^⊤(\bm{y}-\bm{y}_0)$, $\dd{{}^N u}=\dd{{}^N y}$ して結局 $(\bm{x}-\bm{x}_0)^⊤A(\bm{x}-\bm{x}_0)$ $= ∑_j (1+iλ_j)(u_j-u_{0j})^2$. 最後に $\det{A} = \det{A_{\mathrm{R}}} ∑_j (1+iλ_j)$ に注意[^cholesky]して積分. Cholesky 分解できるのは $A_{\mathrm{R}}$ が正定値であるとき.

- $\displaystyle ∫\dd{^N x}\ e^{-\frac12 \bm{x}^⊤ A \bm{x} + \bm{J}^⊤ \bm{x}} = \sqrt{\frac{(2π)^N}{\det A}} e^{\frac12 \bm{J}^⊤ A^{-1} \bm{J}}. \quad$ ($A$ : $N×N$ 正則行列で対角化可能)  
    ∵ 平方完成 $-\frac12 \bm{x}^⊤ A \bm{x} + \bm{J}^⊤ \bm{x}$ $= -\frac12 (\bm{x}-A^{-1}\bm{J})^⊤ A (\bm{x}-A^{-1}\bm{J})$ $+ \frac12 \bm{J}^⊤ A^{-1} \bm{J}$ して積分.

- $\displaystyle ∫\dd{^N x}\ e^{-\frac12 \bm{x}^⊤ A \bm{x} + i\bm{J}^⊤ \bm{x}} = \sqrt{\frac{(2π)^N}{\det A}} e^{- \frac12 \bm{J}^⊤ A^{-1} \bm{J}}. \quad$ ($A$ : $N×N$ 正則行列で対角化可能)
...
多変数の Gauss 積分は
$$
∫\dd{^N x}\ e^{-\frac12 (\bm{x}-\bm{x}_0)^⊤ A (\bm{x}-\bm{x}_0)} = \sqrt{\frac{(2π)^N}{\det A}}, \quad (\det{A}≠0)
$$
である.
...
$$
∫ \mathcal{D}x(t)\ e^{-\frac12 ∫ \dd{t} \{x(t)\}^2} = 1
$$
である. 実際, 多変数 Gauss 積分で
$$
∫\dd{^N x}\ e^{-\frac12 Δt |\bm{x}|^2} = \pqty{\frac{2π}{Δt}}^{N/2}
$$
が成立するが, あるいは書き直して
$$
\frac1{(2π/Δt)^{N/2}} ∫\dd{x_1} \cdots ∫\dd{x_N} e^{-\frac12 Δt (x_1^2 + \cdots + x_N^2)} = 1
$$
が成立するから, [汎関数積分の計算法](./functional.md#汎関数積分)を思い出せば $N→∞$ の極限で汎関数積分になる.
...
汎関数 Gauss 積分は
$$
∫ \mathcal{D}x(t)\ e^{-\frac12 ∫ \dd{t} \{x(t)-x_0(t)\}\^A\{x(t)-x_0(t)\}} = \sqrt{\frac1{\operatorname*{Det}{\^A}}}
$$
である.
...
複素数 $z$ に関する最も簡単な Gauss 積分は
$$
∫\dd{{}^2 z}\ e^{-\frac12 z^*z} = 2π
$$
である. ただし, 積分測度 $\dd{{}^2z}$ は $ℂ$ 平面の面積要素であり, $z=x+iy$ に対して $\dd{{}^2z}=\dd{x}\dd{y}$ で定義される. 実際,
$$
∫\dd{{}^2 z}\ e^{-\frac12 z^*z} = ∫\dd{x}∫\dd{y}\ e^{-\frac12 (x^2+y^2)} = 2π
$$
となる.

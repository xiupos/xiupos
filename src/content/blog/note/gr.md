...
$$
と書き表せる. さて, 有限変位を $Δ\bm{x}→\dd{\bm{x}}$, $Δx^β(\bm{x}')→\dd{x^β(\bm{x}')}$ と無限小にして, 共変微分を
$$
...
$$
と定義する. つまり, $\dd{\bm{x}} = \dd{x^β}\,\bm{e}_β(\bm{x})$ に対して
$$
\dd{\bm{u}} = \dd{x^β} ∇_β\bm{u} = ∇_βu^μ \dd{x^β} \bm{e}_μ(\bm{x})
$$

...
$$
\begin{aligned}
  F[\varphi(x)] &= F[0] + ∫ \dd{x} \fdv{F}{\varphi(x)} \varphi(x) + \frac12 ∫∫ \dd{x} \dd{y} \frac{δ^2F}{δ\varphi(x) δ\varphi(y)} \varphi(x) \varphi(y) + ⋯ \\
  &= ∑_{i = 0}^∞ ∏_n
\end{aligned}
$$
...
体 $K$ 上の加群を $K$ 上の**ベクトル空間**という. または, 集合 $V$ が, **加法**と呼ばれるその上の二項演算子 $+$ と, **スカラー乗法**と呼ばれる体 $K$ の $V$ への作用 $∘$ を持ち, $u, v, w \in V$, $a, b \in K$ に関して以下の公理系を満たすとき, $(V, +, ∘)$ を $K$ 上の**ベクトル空間**という. $V$ をベクトル空間と呼ぶこともある. ベクトル空間 $V$ の元を**ベクトル**と呼ぶ.
...
複素数体 $ℂ$ 上のベクトル空間 $H$ のベクトルを $|\varphi⟩$ と書き, **ケットベクトル**と呼ぶ. また, 双対空間 $H^{*}$ のベクトルを $⟨\varphi| := ⟨(|\varphi⟩), \ ⟩$ と書き, **ブラベクトル**と呼ぶ. これらの記法を用いて, ベクトル $|\varphi⟩, |ψ⟩ ∈ H$ の内積は $⟨\varphi ∣ ψ⟩$ $⟨\varphi ∣ ψ⟩$ と書く. 例えば, $H$ の基底 $\{ |m⟩ \}$ とその双対基底 $\{ ⟨n| \}$ は $⟨n ∣ m⟩ = δ_{nm}$ を満たす. また, 双対写像はブラベクトルに右から作用する: $A^† ∈ \mathrm{End}_K (H^{*})$ で $⟨(A |\varphi⟩), \ ⟩ = ⟨\varphi| A^†$. 線形変換 $A ∈ \mathrm{End}_ℂ (H)$ が Hermite 変換, つまり $⟨\varphi| (A |ψ⟩) = (⟨\varphi| A) |ψ⟩$ であるとき, これを単に $⟨A⟩{ψ}$ と書く. また, $|\varphi⟩^† := ⟨\varphi|$, $⟨\varphi|^† := |\varphi⟩$ と定義すれば $(A |\varphi⟩)^† = ⟨\varphi| A^†$ が得られる.

複素数体 $ℂ$ 上のベクトル空間 $H$ の基底 $\{ |n⟩ \}$ に対し, 線形写像 $|n⟩ ⟨n|$ を**射影写像**という: ケットベクトル $|\varphi⟩ = ∑_m \varphi_m |m⟩$ に対し, $|n⟩ ⟨n | \varphi⟩ = \varphi_n |n⟩$. また, $∑_n |n⟩ ⟨n| = 1_H$ である. 線形変換 $A \in \mathrm{End}_ℂ (H)$ の固有値 $\{ a_n \}$ にそれぞれ属する固有ベクトル $\{ |a_n⟩ \}$ はベクトル空間 $H$ の基底であり, $A$ は射影写像 $|a_n⟩ ⟨a_n|$ の線形結合で表される: $A = ∑_n a_n |a_n⟩ ⟨a_n|$.
...
観測により物理状態が $|ψ⟩$ から $|\varphi⟩$ に遷移する確率は $|⟨\varphi ∣ ψ⟩|^2$ で与えられ, $⟨\varphi ∣ ψ⟩$ を**遷移振幅**という.
...
$$
\begin{aligned}
  \{ q_i, p^j \}_\mathrm{P} &= δ_i^j, & &\leftrightarrow & - \frac{i}{ℏ} [\^q_i, \^p^j] &= δ_i^j, \\
  \{ q_i, q_j \}_\mathrm{P} &= 0, & &\leftrightarrow & - \frac{i}{ℏ} [\^q_i, \^q_j] &= 0, \\
  \{ p^i, p^j \}_\mathrm{P} &= 0, & &\leftrightarrow & - \frac{i}{ℏ} [\^p^i, \^p^j] &= 0. \\
\end{aligned}
$$
...
点 $p^i ∈ U_i ⊂ M$ における局所自明化 $\varphi_i$ を $\varphi_{i,p} := \varphi_i(p,\ ) : F → π^{-1}(p)$ とする. 底空間上の点 $p ∈ U_i ∩ U_j ≠ \varnothing$ について, $g_{ij} (p) := \varphi_{i,p}^{-1} ∘ \varphi_{j,p} : F → F$ あるいは $g_{ij} (p)$ を**変換関数** transition function といい, $p ∈ U_i \cup U_j \cup U_k$ に対してコサイクル条件 $g_{ij} (p) g_{jk} = g_{ik}$ を満たす.
$$
\xymatrix{
...
$$
D \phi := \varphi_{i,p} (\dd{} + A_i) \varphi_{i,p}^{-1} ∘ \phi.
$$
ここで, Lie 代数に値を取る $1$-形式 $A_i ∈ Ω^1 (U_i, \mathrm{End} (E)) = Γ(T^{*}U_i ⊗ \frak{g})$ は**接続 $1$-形式**または**ゲージ場** gauge field といい, **局所標構場** local frame field と呼ばれる $Ω (U_i, E) = Γ(π^{-1} (U_i))$ の局所的な基底 $\{ e_a \}$ を用いて, $∇  e_a = \varphi_{i,p} {(A_i)^b}_a ⊗ \varphi_{i,p}^{-1} ∘ e_b$ と展開できる. また, ゲージ場は別の座標近傍と「接続」する役割を持つ: $p ∈ M$ の座標近傍 $U_i, U_j ⊂ M$ とその局所自明化 $\varphi_{i,p} := \varphi_i (p,\ )$, $\varphi_{j,p} := \varphi_j (p,\ )$ に対し, 切断 $\phi ∈ Γ(E)$ は
$$
D \phi = \varphi_{i,p} (\dd{} + A_i) \varphi_{i,p}^{-1} ∘ \phi = \varphi_{j,p} (\dd{} + A_j) \varphi_{j,p}^{-1} ∘ \phi,
$$
あるいは 局所切断 $\phi_i := \varphi_{i,p}^{-1} ∘ \phi$, $\phi_j  := \varphi_{j,p}^{-1} ∘ \phi$ と, それらに対する局所的な接続 $D_i := \dd{} + A_i$, $D_j := \dd{} + A_j$ を用いて, 変換関数による局所的な接続の変換式が得られる:
$$
...
$$
\begin{aligned}
\varphi_{j,p} (\dd{} + A_j) \varphi_{j,p}^{-1} ∘ \phi
&= \varphi_{j,p} \dd{(\varphi_{j,p}^{-1} ∘ \phi)} + \varphi_{j,p} A_j \varphi_{j,p}^{-1} ∘ \phi \\
&= \varphi_{j,p} \dd{(\varphi_{j,p}^{-1} ∘ \varphi_{i,p} ∘ \varphi_{i,p}^{-1} ∘ \phi)} + \varphi_{j,p} A_j \varphi_{j,p}^{-1} ∘ \phi \\
&= \varphi_{j,p} \dd{(g_{ji} (p) \varphi_{i,p}^{-1} ∘ \phi)} + \varphi_{j,p} A_j \varphi_{j,p}^{-1} ∘ \phi \\
&= \varphi_{j,p} \dd{(g_{ji} (p))} \varphi_{i,p}^{-1} ∘ \phi + \varphi_{j,p} g_{ji} (p) \dd{(\varphi_{i,p}^{-1} ∘ \phi)} + \varphi_{j,p} A_j \varphi_{j,p}^{-1} ∘ \phi \\
&= \varphi_{i,p} \dd{(\varphi_{i,p}^{-1} ∘ \phi)} + \varphi_{i,p} g_{ij} (p) \dd{(g_{ji} (p))} \varphi_{i,p}^{-1} ∘ \phi + \varphi_{i,p} g_{ij} (p) A_j g_{ji} (p) \varphi_{i,p}^{-1} ∘ \phi \\
&= \varphi_{i,p} (d + g_{ij} (p) \dd{g_{ji} (p)} + g_{ij} (p) A_j g_{ji} (p)) \varphi_{i,p}^{-1} ∘ \phi .
\end{aligned}
$$
これが $\varphi_{i,p} (\dd{} + A_i) \varphi_{i,p}^{-1} ∘ \phi$ と等しい条件は,
$$

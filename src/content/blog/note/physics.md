...
観測量 $B$ について, 任意の状態ベクトル $|ψ⟩$ に対し $⟨a| \^B |ψ⟩ = \^B_A ⟨a ∣ ψ⟩ = \^B_A ψ(a)$ を満たす $\^B_A : ℂ → ℂ$ が存在するとき, 観測量 $A$ に対して $\^B |ψ⟩ ↔ \^B_A ψ(a)$ の対応がある. $B = b$ に属する固有状態 $|b⟩$ に対して $ψ_b(a) := ⟨b ∣ a⟩$ とすれば, $b$ は $\^B_A$ の固有値, $ψ_b(a)$ はそれに属する固有波動関数である:
$$
\^B_A ψ_b(a) = ⟨b| \^B |a⟩ = b ⟨b ∣ a⟩ = b ψ_b(a).
$$
また, 物理量 $B$ の期待値は,
$$
\begin{aligned}

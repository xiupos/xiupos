...
汎関数 $F[φ(x)]$ は, 例えば区間 $I = [a, b]$ での $x$ に対する $φ(x)$ の値を引数に取る関数である. たとえば $F[φ(x)] = ∫_I \dd{x} φ(x)$ など. これはちょうど, $I$ の $N$ 分割 $a = x_0 < ⋯ < x_N = b$ に対し, $φ_n := φ(x_n)$ として, $N$ 個の引数を取る関数 $f_N(φ_1,…,φ_N)$ を $N→∞$ の極限を取ったものと見ることができる: $f_N(φ_1,…,φ_N) \overset{N→∞}{⟶} F[φ(x)]$. 例えば, 等間隔な分割 $x_n := a + n(b-a)/N$, $Δx := (b-a)/N$ に対し, $F[φ(x)] = ∫_I \dd{x} φ(x)$, $φ(x) = x^2$ として,
$$

...
$$
\bqty{- ℏ^2 \laplacian + (mc)^2} ψ(\bm{x},t) = - ℏ^2 \frac{\d{{}^2}}{c^2\d{t^2}} ψ(\bm{x},t).
$$
...
$$
としたとき, 相対論的関係式 $(H/c)^2=p^2+(mc)^2$ を満たす $\bm{α}$, $β$ の条件を考える. 2乗すると,
$$
\begin{aligned}
  \pqty{\frac{H}{c}}^2
    &= (α^ip_i + βmc)(α^ip_i + βmc) \\
...
$$
または左から $β$ をかけ, $(γ^μ)=(γ^0,\bm{γ})≡(β,β\bm{α})$ として
$$
γ^0\frac{H}{c} = \bm{γ}⋅\bm{p} + mc, \quad \{γ^μ,γ^ν\}=g^{μν}.
$$

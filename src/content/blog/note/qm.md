...
$$
\frac{H}{c} = α^1p_1 + α^2p^2 + α^3p_3 + βmc ≡ \bm{α}⋅\bm{p} + βmc,
$$
または両辺に左から $β$ をかけ, $(γ^μ)=(β,β\bm{α})$ として
$$
γ^0\frac{H}{c} = γ^1p_1 + γ^2p_2 + γ^3p_3 + (γ^0)^2mc ≡ γ^ip_i + (γ^0)^2mc
$$
としたとき, 相対論的関係式 $(H/c)^2=p^2+(mc)^2$ を満たす $\bm{α}$, $β$ の条件を考える. 両辺を 2乗すると, 左辺は
$$
\pqty{γ^0\frac{H}{c}}\pqty{γ^0\frac{H}{c}} = (γ^0)^2 \pqty{\frac{H}{c}}^2,
$$
右辺は,
$$
\begin{aligned}
   &\ [γ^ip_i + (γ^0)^2mc][γ^jp_j + (γ^0)^2mc] \\
  =&\ (γ^1)^2p_1^2 + (γ^2)^2p_2^2 + (γ^3)^2p_3^2 + \{γ^i,γ^j\}p_ip_j|_{i≠j} + \{γ^i,γ^0\}p_imc + (γ^0)^4(mc)^2. \\
\end{aligned}
$$
これが関係式を満たす条件は
$$
\{γ^μ,γ^ν\}=δ^{μν}.
$$
したがって, このときの Hamiltonian の関係式は
$$
γ^0\frac{H}{c} = γ^ip_i + mc.
$$
これは $d^2$ 本の連立方程式である.
